...
first some general description of the `MSnbase` data classes in `MSnbase` basic
...
from the original files in a one-dimensional list. Functions like `rtime` and
`msLevel` allow to extract the retention time and MS level, respectively, for
each spectrum and return a `numeric` (or `integer`) vector with the same length
than the number of spectra in the `MSnExp`. In the example below we use the
...
The `fromFile` function can be used to determine from which file (sample) a
specific spectrum in the `MSnExp` object is. This function returns an `integer`
vector, same length than spectra in the experiment, with the file index; the
file names can be accessed with the `fileNames` method. A `MSnExp` object can be
subsetted with `[` and e.g. the index of the spectra that should be retained. In
the code block below we subset our `ms_dsk` object to contain only spectra from
the 3rd file.
...
Note however that there are also dedicated *filter* functions to subset an
...
`filterRt` function to further subset our data to contain only spectra acquired
...
This particular spectrum has thus only 3 peaks.
...
not suggested, since it loads all the data into memory and all MS spectrum
