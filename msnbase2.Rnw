...
\red{Thermo Scientific Proteome Discoverer (Thermo Fisher Scientific)},
...
infrastructure to work with raw mass spectrometry \color{red}{data from MS files
  in mzML, mzXML, mzData or ANDI-MS/netCDF format as well as}\sout{,} quantitative and
...
centroiding of \sout{raw} \color{red}{profile-mode MS} data, or normalisation and imputation of
quantitative data), analysis and visualisation of these data and the
development of novel computational mass spectrometry
methods~\cite{Stanstrup:2019}. \color{red}{Extensive documentation and use cases 
are provided in \textit{package vignettes}~\cite{MSnbaseVignettes} and 
workflows~\cite{xcmsWorkflow}.} Here, we focus on the new developments
...
(for MS level 2 and above), and many more. \texttt{MSnbase}\color{red}{
  ,\textit{via} the \texttt{mzR} package~\cite{Chambers:2012}, allows import of raw mass spectrometry
  data from files in mzML, mzXML, mzdata or ANDI-MS/netCDF format and} provides a
...
In the following sections, we describe \sout{\texttt{MSnbase}'s ability to
  efficiently handle very large mass spectrometry data and experiments and how
  it} \code{red}{how \texttt{MSnbase}} can be used for data processing and
visualisation. We will also illustrate how it makes use of the forward-pipe
operator (\texttt{\%>\%}) defined in the \texttt{magrittr} package. This
operator has proved useful to develop non-trivial analyses by combining
individual functions into easily readable pipelines. \code{red}{An example for
  \texttt{MSnbase}'s ability to efficiently handle also very large mass
  spectrometry experiments (in this case with 5,773,464 spectra in 1,182 mzXML
  files) is provided as Supplementary Information.}
...
demonstrates how to create two \texttt{MSnExp} objects \color{red}{importing
  data from an mzML file and} storing \sout{data}\color{red}{it} 
...
\color{red}{Because the on-disk backend does not hold all the data in memory,
  direct data manipulations are not possible. We thus implemented a \textit{lazy
  processing} mechanism in this backend that caches any data manipulation
operation in a processing queue in the object itself. Only when the user
accesses m/z or intensity values these operations are applied}.
\sout{The on-disk backend also offers efficient data manipulation by way of
...
when the user accesses m/z or intensity values. \color{red}{As an additional
  advantage, operations on subsets of the data become much faster since data
  manipulations are applied only to data subsets instead of the full data set
  at once. Also, data access in on-disk backends is parallelized by data file
  ensuring a higher performance of this backend over conventional in-memory data
  representations.} 
As an example, the
...
further analysed in R or written back to disk as an \texttt{mzML} file
using \texttt{writeMSData()} for processing with other tools.
...
the raw MS profile data \color{red}{imported from an mzML file} for \sout{the} serine and the same data after
