...
was complemented by the on-disk backend, that only load
metadata in memory and accesses the spectra in the original files when
needed. There are two direct benefits using the on-disk
backend. Table \ref{tab:bench1} shows 4-fold faster reading times
...
print(xtable(time), floating = FALSE, file = "tabs/t_read.tex")
print(xtable(sz), floating = FALSE, include.rownames = FALSE, file = "tabs/sz.tex")
@


\begin{table}[ht]
\centering
\subfloat[Reading time (s)]{\label{tab:tread}\scalebox{1}{\input{./tabs/t_read}}}\quad
\subfloat[Object size (MB)]{\label{tab:sz}\scalebox{1}{\input{./tabs/sz}}}
\caption{Reading time (in seconds) (left) and data size in memory (in
...
As shown on figure \ref{fig:filt}, this lazy mechanism si
significantly faster than its application on in-memory data.

\begin{figure}[ht]
  \centering
<<plot_filter, message = FALSE, out.width = '50%', fig.width = 5, fig.height = 3>>=
load("bench_t_filt.rda")
ggplot2::autoplot(t_filt)
@
\caption{Filtering benchmark assessed over 10 interations on in-memory (top) and on-disk (bottom) data containing 6103 MS2 spectra.}
\label{fig:filt}
\end{figure}


The advantageous reading and execution times and memory footprint of
the on-disk backend are possible by avoiding unnecessary access to the
raw data. Once access to the spectra M/Z and intensity values become
mandatory (for example for plotting), then the in-memory backend
becomes more efficient, as illustrated on figure
\ref{fig:access}. This gain is maximal when the whole dataset is the
be accessed (i.e. all spectra are already in memory) and negligeable
when large fractions of the data need to be subset.

\begin{figure}[ht]
  \centering
<<plot_access, message = FALSE, out.width = '75%', fig.width = 5, fig.height = 3>>=
