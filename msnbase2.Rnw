...
There exists a very diverse catalogue of software tools to explore, process and
interpret mass spectrometry data. These range from low level software libraries
that are aimed at programmers to enable development of new applications, to more
user-oriented applications with graphical user interfaces. Example of software
libraries include Java-based jmzML~\cite{Cote:2010} or C/C++-based
ProteoWizard~\cite{Chambers:2012}, that are used as application programming
interfaces. ProteomeDiscoverer (Thermo Scientific), MaxQuant~\cite{Cox:2008} and
PeptideShaker~\cite{Vaudel:2015} are among the most widely used user-centric
applications. They generally provide a limited and fixed set of functionality
featuring often advanced graphical user interfaces.
...
software packages such as Python-based {pyOpenMS}~\cite{Rost:2014},
...
development of novel computational mass spectrometry methods~\cite{Stanstrup:2019}.
...
manage mass spectrometry experiments, storing data either in-memory or
on-disk.
...
memory and accesses the spectra data (i.e. m/z and intensity values) in the original files when
needed. There are two direct benefits using the on-disk backend,
namely faster reading and reduced memory footprint. Figure
...
This new on-disk infrastructure enables analyses also of very large scale
experiments using \texttt{MSnbase}). An example analysis of an intermediate
scale metabolomics experiment (~2000 raw data files) using the
\texttt{xcms}~\cite{Smith:2006} R package, that completely builds on the on-disk infrastructure
of \texttt{MSnbase}, is described in [TODO: add citation once published.] 
...
smoothing, centroiding and m/z refinement. Detailed execution and
description of these operations can be found in the \emph{MSnbase:
...
We have presented here some some important functionality of the
version 2 of the \texttt{MSnbase} package. The new on-disk
infrastructure enables large scale data analyses~\cite{Nothias:2020}
using \texttt{MSnbase} or other packages that rely on it such as
