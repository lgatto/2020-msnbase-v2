...
operations on m/z and intensity values}
\sout{The on-disk backend also offers efficient data manipulation by way of
\textit{lazy processing} in the object itself. Operations on the \sout{raw}\color{red}{MS}
data are stored} in a processing queue and only effectively
\sout{applied}\color{red}{applies them}
