...
applications, to user-oriented applications,  such as ProteomeDiscoverer, MaxQuant, ...

The former are used through application programming interfaces
exclusively, while the latter generally featuring graphical user
interfaces (GUI).

Give examples of libraries re-used in user/gui focused application


In this software note, we are going to focus on metabolomics and
proteomics applications of mass spectrometry.
