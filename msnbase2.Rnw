...
\begin{figure}[ht]
  \centering
<<plot_bench1, out.width = '100%', fig.width = 7, fig.height = 3>>=
...
becomes more efficient, as illustrated on
figure~\ref{fig:access}. This gain is maximal when the whole dataset
is the be accessed (i.e. all spectra are already in memory) and
negligeable when large fractions of the data need to be subset.
