...
\textcolor{red}{with the aim} to improve on existing
\textcolor{red}{protocols} and devise new ones. The complexity and
diversity of mass spectrometry yield complex \textcolor{red}{data} of
considerable size, that require non trivial processing before
producing interpretable results.  \textcolor{red}{The complexity and
...
ProteoWizard~\cite{Chambers:2012}.  \textcolor{red}{Thermo Scientific
...
(for example smoothing and centroiding of \textcolor{red}{profile-mode
...
In the following sections, we describe \textcolor{red}{how
  \texttt{MSnbase}} can be used for data processing and
visualisation. \textcolor{red}{An example of its ability to also
...
readable \textcolor{red}{and elegant} pipelines.
...
\textcolor{blue}{The on-disk backend has become the preferred backend
  for large data manipulation, and the only viable alternative for
  very large data (i.e. larger than the available RAM) and/or when
  several MS levels are to loaded and handled simultaneously. The
  on-disk backend can still prove useful in cases when small MS2-only
  data are to be analysed and will remain available in future versions
  of \texttt{MSnbase}.}
...
that demonstrates this. The simple \textcolor{red}{pipeline} is
...
further analysed \textcolor{red}{using \texttt{MSnbase}} or written
...
statistical analysis \textcolor{red}{of} quantitative proteomics data,
