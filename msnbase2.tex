...
infrastructure to work with raw mass spectrometry \textcolor{red}{data from MS files
...
centroiding of \sout{raw} \textcolor{red}{profile-mode MS} data, or normalisation and imputation of
quantitative data), analysis and visualisation of these data and the
development of novel computational mass spectrometry
methods~\cite{Stanstrup:2019}. \textcolor{red}{Extensive documentation and use cases 
...
(for MS level 2 and above), and many
more. \texttt{MSnbase}\textcolor{red}{, \textit{via} the \texttt{mzR}
  package~\cite{Chambers:2012}, allows import of raw mass spectrometry
  data from files in mzML, mzXML, mzdata or ANDI-MS/netCDF format and}
...
In the following sections, we describe \sout{\texttt{MSnbase}'s ability to
  ...
  it} \textcolor{red}{how \texttt{MSnbase}} can be used for data processing and
...
individual functions into easily readable pipelines. \textcolor{red}{An example for
...
demonstrates how to create two \texttt{MSnExp} objects \textcolor{red}{importing
  data from an mzML file and} storing \sout{data}\textcolor{red}{it} 
...
\textcolor{red}{Because the on-disk backend does not hold all the data in memory,
...
when the user accesses m/z or intensity values. \textcolor{red}{As an additional
...
the raw MS profile data \textcolor{red}{imported from an mzML file} for \sout{the} serine and the same data after
